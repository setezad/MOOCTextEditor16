...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		while(i<carray.length){
			c = Character.toLowerCase(carray[i])  ;
			if(c=='a' || c=='i' || c=='u' || c=='o' || c=='e' || c=='y'){
				if(i<carray.length-1){
					count++;
					temp = Character.toLowerCase(carray[i+1]);
					if(vowels.indexOf(temp)>=0){
						i=i+2;
						while(i<carray.length-1){
							temp = Character.toLowerCase(carray[i]);
							i++;
							if(vowels.indexOf(temp)<0){
								break;
							}
				...
				// the last letter
				else if(carray.length==1 || i==carray.length-1){
					count++; 
					i++;
				}
			}
	...
	public double getFleschScore()
	{
		...
		double score = 206.835 - 1.015*((double)this.getNumWords()/(double)this.getNumSentences())-84.6*((double)this.getNumSyllables()/(double)this.getNumWords());
	    return score;
	}
